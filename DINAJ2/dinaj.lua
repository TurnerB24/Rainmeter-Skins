...
local JACKET_LIMIT, COAT_LIMIT, BIG_COAT_LIMIT, UNIT
-- NOT personal taste - used to distribute descriptors across range of temperatures
local TEMP_MIN, TEMP_MAX = -20, 105
...
local function getMainString( temp )
    ...
    local outerwear = (temp < BIG_COAT_LIMIT) and "big coat" or "coat"
    outerwear = (temp < COAT_LIMIT) and "coat" or "jacket"
...
local function getSubString( temp, unit )
    return string.format("It's %s outside | %i", getTempWord(temp, unit), temp)
end
...
function Initialize()
    -- read settings
    JACKET_LIMIT = tonumber(SKIN:GetVariable("jacket_temp", 45))
    COAT_LIMIT = tonumber(SKIN:GetVariable("coat_temp", 30))
    BIG_COAT_LIMIT = tonumber(SKIN:GetVariable("big_coat_temp", 15))
